...
- **自动挖掘：** 以玩家当前方块位置为中心持续搜索并破坏附近的白名单方块。沿用现有白名单与范围算法；由于没有准星目标类型，“相邻同种方块”算法在此模式下明确不可用。
...
1. 按 **`V`** 打开配置界面。
2. 选择采集、自动挖掘、种植或交互模式；安装 Litematica 后还会显示三个投影模式。
3. 配置当前白名单：
...
5. 执行对应操作：
   - 破坏匹配方块以执行连锁采集。
   - 在自动挖掘模式下靠近白名单方块；模组会持续以玩家当前方块位置为中心搜索，并破坏可触及的结果。
...
| 选项 | 说明 |
| :--- | :--- |
| **连锁模式** | 采集、自动挖掘、种植、交互或三个可选 Litematica 模式。 |
| **搜索算法** | 同类相邻、白名单相邻、球形、平面正方形或长方体。自动挖掘不支持同类相邻；平面和长方体搜索强制以玩家为中心。 |
