...
2. **模组服务器（已安装模组）：**
   ...
   - 服务端让各玩家的活动请求公平共享全服单 tick 预算，并支持**直接进入背包**。
...
- `/chainvein maxBlocks [1..2048]`：服务端每 tick 在所有玩家活动请求之间合计处理的最大坐标数量，默认 `256`。
- `/chainvein pickupRadius [0..64]`：直接进入背包与可选快捷潜影盒溢出收纳捕获掉落物的半径，默认 `10`；设为 `0` 可关闭服务端掉落物捕获。

每个协议请求最多包含 `2048` 个坐标。活动玩家会公平分享单 tick 预算，短请求无法用完的份额会重新分配。服务端每 tick 只接受每名玩家的第一份请求；tick 末会先处理该玩家手上的旧请求，再用本 tick 接受的新请求替换旧请求尚未开始的部分。因此每名玩家始终只有一个活动请求，服务端不会形成可无限增长的任务队列。
...
服务端未安装本模组时，挖掘请求会遵循原版破坏进度逐个处理方块。新请求会等待当前方块完成，然后替换旧请求中尚未开始的部分；玩家再次手动挖掘则会丢弃全部剩余 fallback 作业。当前方块与整批请求的进度显示在 ChainVein 原有的顶部居中状态区，不占用打印机类模组常用的准星进度区域。
